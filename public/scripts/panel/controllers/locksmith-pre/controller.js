...
.controller('panelplacePreController', function($timeout, placesFactory,NgMap, $scope, $rootScope, $http, $location, $route, $routeParams) {
  ...
  angular.element(document).ready(function() {
    ...
    $scope.onLatLonInputChange = function(e) {
        if(!invalidForm()){

         $scope.lock.latitude = parseFloat($scope.lock.latitude);
         $scope.lock.longitude = parseFloat($scope.lock.longitude);
         $scope.lock.position = [ $scope.lock.latitude ,$scope.lock.longitude];
          $scope.positions = [];
          $scope.positions.push($scope.lock);
          $scope.center = [$scope.lock.latitude, $scope.lock.longitude];
  ...
  $scope.clicky = function() {
    ...
    if (confirm("Desea realmente aprobar la peticion de la lugar " + $scope.lock.razon_social + "?")) {

      var geocoder = new google.maps.Geocoder();

      var address = httpdata.direccion + ", " + httpdata.nombre_localidad + ", " + httpdata.nombre_provincia;
      ...
      geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
          ...
          $http.post('../../../api/apr/' + $scope.lock.id, httpdata)
            .then(
              function(response) {

                if (response.data.length == 0) {
                  document.location.href = $location.path() + '../../../apr';
                } else {
